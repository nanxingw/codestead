...
> **当前状态：M0 工程脚手架。** 仓库刚立起骨架——能安装、能跑通工具链、浏览器里有一块
> 640×360 的空画布，**尚无任何玩法**。里程碑规划（M1 农场核心循环 → M2 会话 HUD →
> M3 建造 → M4 AI 关卡 → M5 开源发布）见 CLAUDE.md。
...
打开终端里 Vite 打印的地址（默认 `http://localhost:5173`），能看到一块 640×360、
整数倍缩放的深色像素画布与版本文字——这就是 M0 的全部游戏内容（诚实进度）。
