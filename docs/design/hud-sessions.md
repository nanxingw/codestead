...
>
> **实施分期注记（M2）**：M2 第一版信号源收敛为两源——hooks（语义主路）+ transcript fs.watch（Esc 中断盲区兜底）；ps 轮询、unknown 态展示、§2.2 第 6' 行安装指引、kill -9 收尸推至 M2 末。过渡期幽灵会话以「idle 持续 ≥12 小时摘牌」近似收尸；ps 收尸（行 14）落地后，该近似**仅对未关联 pid 的会话（cwd 采集失败、无法与任何 ps 行对应，见 §12-D2-A3）保留为兜底**——已关联 pid 的存活会话不再被摘牌（showIdle=true 时空闲会话照常显示）；被兜底误摘的会话经 ps 逐轮重发现以 unknown 回归，或在下一个 hook/transcript 信号时回归。本节状态机语义定义原样保留，其中 unknown / ps 相关行分期实现（M2 末）；验收同步拆分见 §13。
...
| `lastSignalAt` | ISO 8601 | 不直接展示（staleness 归 daemon）；tooltip 调试备用 | daemon 对**仅 lastSignalAt 变化**的 `sessionUpsert` 按每会话 ≥5s 节流（心跳帧已维持 liveness，§8.1 STALE 语义不受影响）；HUD 显示层对展示投影做等值比较，仅 lastSignalAt 变化不触发重绘（§5.3 防跳动的工程化） |
...
| 4 | `POST /hooks` 的 Origin 校验 | §4.1-5 字面仅规定 /handshake CORS 与 WS upgrade 校验 Origin | /hooks 不校验 Origin 留下本机网页 no-cors POST（text/plain 免预检）注入伪造 hook 的欺骗面（DNS rebinding 下 Host 亦不可信） | **安全三脚架的 Origin 腿覆盖全部三个端点**：带 Origin 头且不在白名单的 /hooks 请求仍回空 2xx（保持「只听不说」契约）但丢弃 body、不入状态机——本地 CLI 与 Claude Code 的 HTTP hook 客户端不带 Origin，不受影响；另设 WS `maxPayload` 64KB 与 Host 头白名单（仅 127.0.0.1 / localhost）作纵深 | tech-stack §4.1-5 已同步；daemon server 据此实现（M2 末） |
...
| D2 | daemon 子系统 | **A1** staleness/降级/收尸全在服务端（语义按本稿 §7，HUD 不做语义判断）；**A2** `/handshake` 端点 + 43110–43119 探测约定（§10.3 P2，已转正、tech-stack 已同步）；**A3** process-only 会话的 `cwd` 可能为空串：`ps` 本身取不到 cwd，daemon 对每个候选 pid 以 `lsof -p <pid> -a -d cwd`（macOS）/ `readlink /proc/<pid>/cwd`（Linux）尽力采集（每 pid 缓存一次、失败回退空串）——该采集同时是「pid↔会话按 cwd 关联」与 kill -9 收尸（§7.3 行 14）覆盖 hooks 会话的前提；ps 源每轮轮询对全部存活 pid 重发 `processDiscovered`（reducer 对已采纳 pid 幂等 no-op），保证关联与 unknown 注册逐轮重评估；HUD 已设回退链；**A4** headless（quest 生成）会话保证被过滤、永不出现在 snapshot | A1/A2/A4 已定稿；A3 已定稿（M2 末实测：lsof/procfs 采集落地） |
...
| D6 | 菜单/设置子系统 | 「设置 → 会话面板」承载 §9 的 10 个设置项（M2 首版仅暴露 6 项，分期见 §9）+ 连接状态/版本/安装指引展示；实现形态：设置页「会话面板」行（行内连接状态摘要）→ 同名子页（与 §9 标题「设置 → 会话面板」一致），行与子页均不受 everConnected 门控（§8.2） | 已落地（M2 末） |
...
11. **收尸**：进程消失（kill -9 等）后 ps 轮询收尸、不留幽灵会话——前提是 cwd 采集使 pid 关联到 hooks/transcript 会话（§12-D2-A3）；idle ≥12 小时近似仅对 pid 关联失败的会话保留为兜底（§7 分期注记），已关联 pid 的存活空闲会话不受影响；§7.3 转移总表 unknown / ps 相关行补齐回放覆盖（含「canned ps 输出 → 真实 ps 源 → reducer」端到端用例）；
12. **推迟设置项**：showIdle / showUnknown / autoFade / soundInBackground 暴露后按 §9 语义逐项验收。

**验收记录（M2 末，D5 资产落地说明）**：8×8 状态图标 ×7（含 `_hollow` 描边变体与 ◐ 4 帧）已入 ui atlas（自绘 CC0，manifest 可溯源），面板图标改为 atlas 帧渲染（×1/×2/×3 整数缩放锐利，§13-2）；`source==='process'` 的 hollow 修饰由专用描边帧承载（atlas 缺失时降级为字形 + 0.65 调暗，仅资产加载失败的容错路径）；提示音为 Kenney UI Audio `click1`（94ms ≤200ms），播放路径固定 40% 音量（§3.4）。一处实现选择：面板背景仍按 §3.2 色值以 Graphics 矩形绘制——§3.2 规定填充随 opacity 设置、1px 边框恒为不透明，这一「双 alpha」规格单张 9-slice 贴图无法表达；等效的 `hud_panel` 9-slice 资产已入 atlas 供平面 chrome 使用。两种绘制在像素输出上等价（直角、无 AA）。
