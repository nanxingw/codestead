# 18 · NPC 与 AI 关卡（ai-quests）设计稿

> 版本 v1.0 ｜ 2026-06-10 ｜ 状态：草案 ｜ 里程碑：M4
> 上游依据：`00-constitution.md`（最高约束）、`docs/design/tech-stack.md` §4.2/§5、`docs/design/research/daemon.md` §3、`docs/design/research/hooks.md`、`docs/design/research/assets.md` §5、`docs/design/research/mechanics.md` §9。
...
| `npc_grocer` | 阿穗 | 杂货店老板娘，给每件货贴标签 | `naming` `api-design` `interfaces`（命名、API 形状、对外契约、参数设计） | 热络爱举例；执念是「名字不对，货就卖不出去」；会复述你的命名让你自己听一遍 | 小卖部门口（地图南侧） |
| `npc_keeper` | 渠叔 | 水渠管理员，每天巡渠找漏 | `testing` `edge-cases` `reliability` `debugging`（测试策略、边界情况、失败处理、回归） | 谨慎，口头禅「如果……会怎样？」；相信「漏不是补出来的，是巡出来的」 | 水渠闸口（地图西北） |
...
定义在 `packages/daemon/src/quest/local-pool.ts`，首发 12 条（运行时随机不重复抽取，全部抽完后重置）。每条带 NPC 亲和标签：
...
| `cooldownMinutes` | `15` | **15~120** | 两次生成尝试的最小间隔（真实分钟）。下限 15 是宪法硬上限「≤1 个 / 15 真实分钟」，调低属修宪事项 |
...
T3  now - lastAttemptAt >= cooldown             # 含失败尝试，防抖
T4  游戏客户端已通过 WS 认证连接                  # 没人在玩就不花钱、不积压
T5  游戏端不处于对话/日结算屏（gameBusy=false）   # 投放也要挑安静时机
T6  当日 AI 生成次数 < dailyMaxQuests 且当日成本 < dailyBudgetUsd
T7  存在候选会话（见 3.3）→ 走 AI 生成
    不存在候选会话 且 localTemplates → 走本地题库（同样受 T1~T6、同一冷却约束）
T8  aiGeneration == false 时跳过 AI 路径，直接 T7 本地题库
```
...
4. 脱敏摘要后有效文本 < 300 字符 → 视为上下文太薄，**放弃本次 AI 生成**（宁缺毋滥），按 T7 走本地题库或干脆不出。
...
### 4.5 CLI 调用（沿用 tech-stack §1 裁决，不用 `--bare`）
...
- daemon 自管超时：**90s SIGTERM → 再 5s SIGKILL**；
- 启动时 feature-detect（`claude --version` + 关键 flag 探测），不可用则 M4 整体降级（§9），**绝不崩溃**；
...
### 4.6 Quest JSON Schema（`packages/shared/src/quest.ts`，zod v4 单一事实源）
...
### 4.7 WS 协议补充（对 tech-stack §5 草案的修订提案）
...
{ v: 1, type: 'gameBusy',      payload: { busy: boolean } }     // 新增：对话/结算屏开合，供投放时机 T5 用
```
...
- 对话打开 → `gameBusy:true` 上报 daemon（投放时机 T5）；关闭 → false。
...
CLI 配套（daemon 包 `codestead` bin）：`codestead notes list [--session <id>]`、`codestead notes show <questId>`、`codestead notes open`（用系统文件管理器打开 notes 目录）。
...
- M4 阶段玩家日收入基准 200~350 g/日（M1 毕业值起步）；
- 频率上限 1 任务 / 15 真实分钟 = 1 任务 / **5 游戏日**（1 游戏日 = 3 真实分钟）；
- 金币流上限：120g ÷ 5 游戏日 = **24 g/日 ≈ 日收入的 7%~12%**——是「捎带的谢礼」，不是第二条收入线；玩法上永远不值得为刷任务而停止种地；
- XP 流上限：60 ÷ 5 = 12 XP/日，约为同期农耕 XP（≈50~60 XP/日）的 20%，对 Lv6~10 长阈值（3,300→15,000）只是温和助推；
- 本地题库 40g 刻意压到 AI 任务的一半以下：零成本任务不应与取材自真实工作的任务等值，保持 AI 任务的「特别感」；
- 道具奖励均为非生产性（装饰/图鉴/一次性种子），不引入任何持续产出，避免破坏 §4.4 定价公式。
...
| shared 协议 | `quest.ts`（本稿 §4.6 schema 与奖励常量）；`protocol.ts` 新增 `questSnapshot` / `questDismiss` / `gameBusy` 三条消息（§4.7，对 tech-stack §5 草案的修订提案） | zod schema，单一事实源 |
| 游戏渲染（M1 场景） | UIScene 对话框组件、打字机文本、DOM overlay 输入框、NPC 精灵渲染与 E 交互（复用面朝格交互，宪法 §4.5） | 游戏端组件 |
| 经济/物品 | `grantReward({gold, xp, itemId?})` 接口；道具 `seed_pack_t{1..3}`、`deco_thinking_scarecrow`、`collectible_villagers_letter` 三个 id 需在物品表注册（**假设**：物品/图鉴子系统接受非生产性装饰与收藏品类目） | 函数接口 + 数据表 |
| 存档 | save schema 新增 `quests: { grantedQuestIds: string[]; completedCount: number }`（zod 迁移链 +1 版本） | schema 变更 |
| 日循环/日结算屏 | 结算屏「明日预告」区暴露一个可注入文本行的插槽（§6.3）；对话打开 = 模拟 tick 全停（宪法 §4.3，由时间系统保证） | UI 插槽 |
| 地图 | 农场地图预留 3 个 NPC 站位点（木工台/小卖部门口/水渠闸口，Tiled object layer 标记 `npc_spawn`） | 地图标记 |
...
2. **多轮追问**：daemon.md §3 的 `--resume` 支持村民对玩家答案追问一轮（更像对话），但成本与打扰双增——M4 先单轮，是否在 M5 后开实验开关？
